...
export default function ManageLecturerSummariesTab({ allUsers = [] }) {
  ...
  const handlePrintSummary = () => {
    ...
    if (printWindow) {
        ...
        otherClaimsHtml += '</tbody></table>';
        
        if (!hasOtherClaims && selectedClaimType !== "ALL" && selectedClaimType !== "TEACHING") {
             otherClaimsHtml = `<div class="section-title">${selectedClaimType.toLowerCase().replace("_"," ")} Claims</div><p>No ${selectedClaimType.toLowerCase().replace("_"," ")} claims found for this period.</p>`;
        ...
        const printHtml = `<html><head><title>Lecturer Claim Summary - ${summaryData.lecturerName} - ${summaryData.month} ${summaryData.year}</title><style>body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;margin:0;padding:0;color:${textColor};background-color:#fff;font-size:11px;}.print-container{width:100%;max-width:900px;margin:15px auto;padding:20px;}.header{text-align:center;margin-bottom:20px;padding-bottom:15px;border-bottom:2px solid ${uewDeepBlue};}.logo{max-width:90px;margin-bottom:8px;}.university-name{font-size:18px;font-weight:700;color:${uewDeepBlue};margin-bottom:2px;}.college-name{font-size:13px;font-weight:500;color:${uewDeepBlue};margin-bottom:5px;}.document-title{font-size:15px;font-weight:600;color:${uewDeepRed};margin-top:8px;text-transform:uppercase;}.section{margin-bottom:15px;padding:10px;border:1px solid ${lightGrayBorder};border-radius:5px;}.section-title{font-size:13px;font-weight:600;color:${headingColor};margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid ${uewDeepBlue}33;}.summary-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;margin-bottom:12px;}.summary-item{padding:8px;border-radius:4px;text-align:center;border:1px solid ${lightGrayBorder};}.summary-item strong{display:block;font-size:16px;margin-bottom:2px;}.summary-item span{font-size:10px;color:#4A5568;}.total{background-color:${uewDeepBlue}1A;color:${uewDeepBlue};}.pending{background-color:#FFF8E1;color:#E65100;}.approved{background-color:#E8F5E9;color:#1B5E20;}.rejected{background-color:#FFEBEE;color:#C62828;}.footer{text-align:center;margin-top:30px;font-size:10px;color:#555;border-top:1px solid ${lightGrayBorder};padding-top:15px;}.claims-table{width:100%;border-collapse:collapse;margin-top:8px;font-size:10px;table-layout:auto;}.claims-table th,.claims-table td{border:1px solid ${lightGrayBorder};padding:4px 6px;text-align:left;vertical-align:top;word-break:break-word;}.claims-table th{background-color:#eef2f7;font-weight:600;}.status-badge{padding:2px 6px;border-radius:10px;font-weight:500;font-size:0.65em;color:white;text-transform:uppercase;display:inline-block;}.status-PENDING{background-color:#FF8F00;}.status-APPROVED{background-color:#2E7D32;}.status-REJECTED{background-color:${uewDeepRed};}.signature-section{margin-top:40px;padding-top:20px;border-top:1px dashed ${lightGrayBorder};}.signature-area{display:flex;justify-content:space-around;margin-top:25px;}.signature-block{text-align:center;width:30%;}.signature-line{border-bottom:1px solid #333;height:40px;margin-bottom:5px;}.signature-label{font-size:10px;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;font-size:9pt;}.print-container{width:100%;margin:0 auto;padding:10mm;box-shadow:none;border:none;}.status-badge,.summary-item{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="print-container"><div class="header"><img src="/uew.png" alt="University Logo" class="logo" /><div class="university-name">UNIVERSITY OF EDUCATION, WINNEBA</div><div class="college-name">COLLEGE OF DISTANCE AND e-LEARNING (CODeL)</div><div class="document-title">Lecturer Monthly Claim Summary</div></div><div class="section"><div class="section-title">Summary For</div><p><strong>Lecturer:</strong> ${summaryData.lecturerName} (${summaryData.lecturerEmail || 'N/A'}) ${summaryData.lecturerDesignation ? '- ' + summaryData.lecturerDesignation.replace(/_/g, " ") : ''}</p><p><strong>Period:</strong> ${summaryData.month}, ${summaryData.year}</p>${selectedClaimType !== "ALL" ? `<p><strong>Filtered by Claim Type:</strong> <span style="text-transform:capitalize;">${selectedClaimType.toLowerCase().replace("_"," ")}</span></p>` : ''}</div><div class="section"><div class="section-title">Overall Statistics for Period</div><div class="summary-grid"><div class="summary-item total"><strong>${summaryData.totalClaims}</strong><span>Total Claims</span></div><div class="summary-item pending"><strong>${summaryData.pending}</strong><span>Pending</span></div><div class="summary-item approved"><strong>${summaryData.approved}</strong><span>Approved</span></div><div class="summary-item rejected"><strong>${summaryData.rejected}</strong><span>Rejected</span></div></div>${summaryData.totalTeachingHours > 0 ? `<p><strong>Total Approved Teaching Hours:</strong> ${summaryData.totalTeachingHours.toFixed(1)} hrs</p>` : ''}${summaryData.totalTransportAmount > 0 ? `<p><strong>Total Approved Transport Amount:</strong> GHS ${summaryData.totalTransportAmount.toFixed(2)}</p>` : ''}</div>${teachingDetailsTableHtml ? `<div class="section">${teachingDetailsTableHtml}</div>` : ''}${teachingTransportTableHtml ? `<div class="section">${teachingTransportTableHtml}</div>` : ''}${otherClaimsHtml ? `<div class="section">${otherClaimsHtml}</div>` : ''}<div class="signature-section"><div class="signature-area"><div class="signature-block"><div class="signature-line"></div><p class="signature-label">Prepared by</p></div><div class="signature-block"><div class="signature-line"></div><p class="signature-label">Authorized by</p></div><div class="signature-block"><div class="signature-line"></div><p class="signature-label">Approved by</p></div></div></div><div class="footer">Generated on: ${new Date().toLocaleString('en-US', dateTimeLocaleStringOptions)} by System.</div></div></body></html>`;
