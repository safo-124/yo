...
export async function assignCoursesToLecturerByCoordinator({ lecturerId, courseIds, coordinatorId }) {
  ...
  console.log(`[${timestamp}] [assignCoursesToLecturerByCoordinator] Action called by Coordinator ${coordinatorId} for Lecturer ${lecturerId}`);

  if (!lecturerId || !Array.isArray(courseIds) || !coordinatorId) {
  ...
  try {
    ...
    // 2. Find the lecturer and ensure they belong to the coordinator's center
    ...
    if (!lecturer) {
      return { success: false, error: "Lecturer not found in your center." };
    }
    ...
    if (courseIds.length > maxCoursesAllowed) {
      return { 
        success: false, 
        error: `Cannot assign more than ${maxCoursesAllowed} courses to a lecturer as set by registry. Please remove some courses or contact registry to increase the limit.`,
        maxCoursesAllowed
      };
    }
    ...
    console.log(`[${timestamp}] [assignCoursesToLecturerByCoordinator] Successfully assigned ${courseIds.length} courses to ${lecturer.name}.`);
    ...
    return { success: true, message: `Successfully assigned ${courseIds.length} courses to ${lecturer.name}.` };

  } catch (error) {
    console.error(`[${timestamp}] [assignCoursesToLecturerByCoordinator] Error:`, error);
...
export async function getAssignmentPageData(centerId) {
  const timestamp = new Date().toISOString();
  console.log(`[${timestamp}] [getAssignmentPageData] Fetching data for center: ${centerId}`);
  ...
  try {
    ...
    console.error(`[${timestamp}] [getAssignmentPageData] Error:`, error);
