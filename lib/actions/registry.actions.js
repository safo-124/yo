  ...
  try {
    ...
    const updatedUserResult = await prisma.$transaction(async (tx) => {
        if (centerToUnassignCoordinator) {
            await tx.center.update({ where: { id: centerToUnassignCoordinator }, data: { coordinatorId: null }});
  ...
  try {
    ...
    const finalSummary = Object.values(summaryByCenter).map(c => ({
      ...
      departments: Object.values(c.departments).map(d => ({
        ...d,
        lecturers: Object.values(d.lecturers).map(l => ({ ...l, courses: Object.values(l.courses).sort((a,b) => a.courseCode.localeCompare(b.courseCode)) })),
