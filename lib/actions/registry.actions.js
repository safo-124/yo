...
export async function updateProgram({ id, programCode, programTitle, programCategory, departmentId }) {
  const timestamp = new Date().toISOString();
  console.log(`[${timestamp}] [updateProgram] Action called for ID: ${id}`);

  if (!id || !programCode?.trim() || !programTitle?.trim() || !programCategory || !departmentId) {
    return { success: false, error: "All program fields are required for update." };
  }
  ...
  try {
    ...
    revalidatePath('/registry'); // Revalidate the main registry path to update all tabs
...
export async function updateDepartment({ id, name, centerId }) {
  const timestamp = new Date().toISOString();
  console.log(`[${timestamp}] [updateDepartment] Action called for ID: ${id}`);

  if (!id || !name?.trim() || !centerId) {
    return { success: false, error: "Department ID, name, and Center ID are required." };
  }
  ...
  try {
    ...
    // Revalidate paths to refresh data on the client side
    revalidatePath('/registry'); // A general path to cover all tabs
    ...
    if (error.code === 'P2002') {
      return { success: false, error: `A department with this name may already exist in the selected center.` };
    }
