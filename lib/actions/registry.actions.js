...
export async function bulkUploadCourses(courseDataArray) {
  ...
  for (const rawCourse of courseDataArray) {
    try {
      ...
      if (!rawCourse.courseCode?.trim() || !rawCourse.courseTitle?.trim() || rawCourse.creditHours == null || !rawCourse.level || !rawCourse.academicSemester || !rawCourse.programId) {
      ...
      // Check if program exists for this course
      const programExists = await prisma.program.findUnique({
        where: { id: rawCourse.programId },
        select: { id: true },
      });
      if (!programExists) {
        failedRecords.push({ data: rawCourse, error: "Program not found." });
        continue;
      }
      ...
      const createdCourse = await prisma.course.create({
        data: {
          ...
          program: { connect: { id: rawCourse.programId } },
        },
      });
      ...
      if (error.code === 'P2002') {
        errorMessage = `Duplicate course entry (Code: ${rawCourse.courseCode}, Program: ${rawCourse.programId}, Level: ${rawCourse.level}, Semester: ${rawCourse.academicSemester}) already exists.`;
      }
