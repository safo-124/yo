...
// ---getPotentialCoordinators, createCenter, getCenters, createUserByRegistry, getAllUsers, getAllClaimsSystemWide, processClaimByRegistry, updateUserRoleAndAssignmentsByRegistry, updateUserPasswordByRegistry, getPublicCenters, getPendingSignupRequests ---
// ... (These functions remain unchanged from the version you last provided) ...
// Assume all other functions are present and correct as per your last complete file paste.
// I will only show the modified functions: approveSignupRequest and rejectSignupRequest.
    ...
    if (existingUser) {
      await prisma.signupRequest.update({
        ...
        data: {
          ...
          // notes: "User with this email already exists.", // MODIFIED: Removed notes
          registryProcessor: { // This assumes 'registryProcessor' is the correct relation name
    ...
    const newUser = await prisma.$transaction(async (tx) => {
      ...
      await tx.signupRequest.update({
        ...
        data: {
          status: 'APPROVED',
          processedAt: new Date(),
          // notes: "User account created.", // MODIFIED: Removed notes
          registryProcessor: { // This assumes 'registryProcessor' is the correct relation name
            connect: { id: registryUserId }
          }
        },
        ...
        try {
            await prisma.signupRequest.update({
                ...
                data: {
                    status: 'REJECTED',
                    processedAt: new Date(),
                    // notes: "User email exists (transaction).", // MODIFIED: Removed notes
                    registryProcessor: { // This assumes 'registryProcessor' is the correct relation name
                      connect: { id: registryUserId }
                    }
                },
  ...
  try {
    ...
    await prisma.signupRequest.update({
      ...
      data: {
        status: 'REJECTED',
        processedAt: new Date(),
        // notes: rejectionReason, // MODIFIED: Removed notes (rejectionReason could be logged or handled differently if needed)
        registryProcessor: { // This assumes 'registryProcessor' is the correct relation name
          connect: { id: registryUserId }
        }
      },
    });
    console.log(`[${timestamp}] Request ${requestId} rejected. The reason was: ${rejectionReason}`); // Log reason if not saving to DB
    revalidatePath('/registry/requests');
  ...
  try {
    ...
    const finalSummary = Object.values(summaryByCenter).map(c => ({
      ...
      departments: Object.values(c.departments).map(d => ({
        ...d,
        lecturers: Object.values(d.lecturers).map(l => ({ ...l, courses: l.courses ? Object.values(l.courses).sort((a,b) => (a.courseCode || "").localeCompare(b.courseCode || "")) : [] })),
...
export async function deleteUserByRegistry({ userIdToDelete, registryUserId }) {
  ...
  try {
    ...
    if (error.code === 'P2003') { 
        if (error.meta?.field_name?.includes('Claim_submittedByIdToUser')) return { success: false, error: `Cannot delete user. They have submitted claims. Reassign or delete claims first.`};
        if (error.meta?.field_name?.includes('Claim_processedByIdToUser')) return { success: false, error: `Cannot delete user. They have processed claims. Reassign or delete claims first.`};
        if (error.meta?.field_name?.includes('SupervisedStudent_supervisorIdToUser')) return { success: false, error: `Cannot delete user. They are listed as a supervisor for students. Update student records first.`};
        return { success: false, error: `Cannot delete user. Linked to other records (Field: ${error.meta?.field_name}). Resolve dependencies.`};
    }
...
export async function deleteCenterByRegistry({ centerId, registryUserId }) {
  ...
  try {
    ...
    if (error.code === 'P2003') { 
        if (error.meta?.field_name?.includes('Department_centerIdToCenter')) return { success: false, error: `Cannot delete center. It has associated departments. Delete departments first.`};
        if (error.meta?.field_name?.includes('Claim_centerIdToCenter')) return { success: false, error: `Cannot delete center. It has associated claims. Reassign or delete claims first.`};
        return { success: false, error: `Cannot delete center. Linked to other records (Field: ${error.meta?.field_name}). Resolve dependencies.`};
    }
    if (error.message.includes("associated claims")) { 
