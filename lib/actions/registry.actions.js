    ...
    if (lecturerName) {
      whereClause.submittedBy = {
        name: {
          ...
          // For MySQL, `mode: 'insensitive'` might cause issues if the column collation
          // is already case-insensitive (default). Prisma might throw an error.
          // If your User.name column has a case-insensitive collation, `mode: 'insensitive'` is not needed.
          // If it's case-sensitive and you need this, ensure your DB setup supports it.
        }
      };
    }
...
export async function processClaimByRegistry({ claimId, status, registryUserId }) {
  ...
  try {
    ...
    // Optional: Allow processing only if PENDING?
    // if (claimToUpdate.status !== 'PENDING') {
    //   console.log(`[${new Date().toISOString()}] [processClaimByRegistry] Info: Claim ${claimId} is already ${claimToUpdate.status}. Processing anyway.`);
    // }
...
export async function updateUserRoleAndAssignmentsByRegistry({ userId, newRole, newCenterId, newDepartmentId }) {
  ...
  try {
    ...
    if (userToUpdate.Center_Center_coordinatorIdToUser && (newRole === 'LECTURER' || newRole === 'REGISTRY')) {
        ...
        // TODO: Implement logic to handle reassignment or clearing of coordinator for userToUpdate.Center_Center_coordinatorIdToUser.id
    }
...
export async function updateUserPasswordByRegistry({ userId, newPassword }) {
  ...
  try {
    ...
    // Add policy: e.g., REGISTRY cannot change another REGISTRY's password unless specific conditions are met.
    // For now, allowing it.
 ...
 * Includes requested center name by fetching all centers and mapping.
 */
...
export async function approveSignupRequest({ requestId, registryUserId }) {
  ...
  try {
    ...
    if (error.code === 'P2002' && error.meta?.target?.includes('email')) {
        // Attempt to mark request as rejected even if user creation failed due to unforeseen P2002 on email
        try {
            await prisma.signupRequest.update({
                where: { id: requestId },
                data: { status: 'REJECTED', processedAt: new Date(), processedByRegistryId: registryUserId },
