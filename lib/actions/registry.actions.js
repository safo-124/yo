...
export async function createUserByRegistry({ name, email, password, role, designation, lecturerCenterId, departmentId }) {
  const timestamp = new Date().toISOString();
  ...
  try {
    ...
    if (error.code === 'P2002') return { success: false, error: "User with this email already exists (DB constraint)." };
...
export async function updateUserRoleAndAssignmentsByRegistry({ userId, newRole, newDesignation, newCenterId, newDepartmentId, newStaffRegistryCenterIds }) {
  const timestamp = new Date().toISOString();
  ...
  try {
    ...
    if (error.code === 'P2002' && error.meta?.target?.includes('userId_centerId')) return { success: false, error: "Failed to update staff center assignments due to a conflict."};
    ...
    const newUser = await prisma.$transaction(async (tx) => {
      const createdUser = await tx.user.create({
        data: {
          ...
          // designation: request.requestedDesignation || null, // If 'requestedDesignation' is on SignupRequest schema
          lecturerCenterId: request.requestedRole === 'LECTURER' ? request.requestedCenterId : null,
          approvedSignupRequestId: request.id,
        },
        select: { id: true, name: true, email: true, role: true, designation: true, lecturerCenterId: true, createdAt: true, Center_User_lecturerCenterIdToCenter: request.requestedRole === 'LECTURER' && request.requestedCenterId ? { select: { name: true } } : undefined }
      });
