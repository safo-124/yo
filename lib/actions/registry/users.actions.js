...
export async function deleteUserByRegistry({ userIdToDelete, registryUserId }) {
  ...
  try {
    ...
    // If deleting another REGISTRY user, ensure at least one REGISTRY user remains
    if (userToDelete.role === 'REGISTRY') {
      const registryCount = await prisma.user.count({ where: { role: 'REGISTRY' } });
      if (registryCount <= 1) {
        return { success: false, error: "Cannot delete the last REGISTRY user. At least one must remain." };
      }
    }
