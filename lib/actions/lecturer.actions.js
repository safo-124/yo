  ...
  try {
      ...
      select: {
        ...
        Claim_Claim_submittedByIdToUser: { // Relation from User to Claims they submitted
          ...
          select: {
            ...
            updatedAt: true,
            processedAt: true,
            // Include specific fields for quick summary if needed, or fetch full details on demand
            teachingHours: true, // Example
            transportAmount: true, // Example
            thesisType: true, // Example
            // You might want to include the processing coordinator's name
            User_Claim_processedByIdToUser: { // Coordinator who processed
                select: { name: true }
            }
          }
        }
    ...
    const formattedData = {
      ...
      claims: lecturer.Claim_Claim_submittedByIdToUser.map(claim => ({
          ...claim,
          processedByCoordinator: claim.User_Claim_processedByIdToUser?.name
      })),
    };
...
export async function submitNewClaim(claimData) {
  ...
  // Add more specific validations for TRANSPORTATION and THESIS_PROJECT types
  ...
  try {
    ...
    // Revalidate paths where claims are displayed
    revalidatePath(`/lecturer/${submittedById}/claims`); // Lecturer's claim list
    revalidatePath(`/coordinator/${centerId}/claims`); // Coordinator's claim list for that center
