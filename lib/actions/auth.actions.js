...
export async function requestSignup({ name, email, password, role, requestedCenterId, requestedDesignation }) {
  ...
  try {
    ...
    const signupData = {
      ...
      requestedRole: role,
      // If your SignupRequest model is updated to include requestedDesignation:
      // requestedDesignation: requestedDesignation || null, 
      status: 'PENDING',
    };
    ...
    // For STAFF_REGISTRY or COORDINATOR, requestedCenterId is not set here as their
    // center assignments are typically handled post-approval by an admin.
    ...
    // Example for handling designation enum error if it were part of SignupRequest model
