...
export async function loginUser(credentials) {
  ...
  try {
    const user = await prisma.user.findUnique({
      where: { email },
      // Include lecturerCenterId directly if not already selected by default or if needed for other logic
      // select: { id: true, email: true, name: true, role: true, password: true, lecturerCenterId: true }
    });
    ...
    const sessionPayload = {
      ...
      name: user.name,
      role: user.role,
      // If lecturerCenterId is consistently needed in the session, add it here.
    ...
    if (user.role === 'REGISTRY') {
      ...
      // For LECTURER, redirect to a path including their centerId
      if (user.lecturerCenterId) {
        // Assuming the lecturer's dashboard will be under a path like /lecturer/center/[centerId]/dashboard
        redirectTo = `/lecturer/center/${user.lecturerCenterId}/dashboard`;
      } else {
        // If a lecturer is not assigned to a center, this is an issue.
        // They might not have a specific dashboard to go to.
        console.warn(`Lecturer ${user.email} (ID: ${user.id}) is not assigned to any center. Redirecting to default or info page.`);
        redirectTo = '/lecturer/assignment-pending'; // Or just '/' or '/unauthorized'
        // You might want to create an 'assignment-pending' page to inform the lecturer.
      }
    }
