  ...
  try {
    ...
    if (user.role === 'REGISTRY') {
      ...
      if (user.lecturerCenterId) {
        ...
        console.log(`Lecturer ${user.email} using direct center assignment: ${user.lecturerCenterId}`);
        ...
        if (departmentAssignments?.length > 0) {
          ...
          if (centerAssignments?.length > 0) {
            ...
            console.log(`[AUTH] Lecturer ${user.email} course assignment data:`, {
            ...
            console.log(`Lecturer ${user.email} has ${user.lecturerCourseAssignments.length} course assignments. Using center ${centerId} from first assignment.`);
            dashboardPath = `/lecturer/center/${centerId}/dashboard`;
          } else {
            console.warn(`Lecturer ${user.email} course department has no center assignments. Redirecting to assignment-pending.`);
            dashboardPath = '/lecturer/assignment-pending';
          }
        } else {
          console.warn(`Lecturer ${user.email} course program has no department assignments. Redirecting to assignment-pending.`);
          dashboardPath = '/lecturer/assignment-pending';
        }
      } else {
        console.warn(`Lecturer ${user.email} (ID: ${user.id}) is not assigned to any center or courses. Redirecting to assignment-pending.`);
        dashboardPath = '/lecturer/assignment-pending';
      }
    }
...
export async function getSession() {
  const cookieStore = await cookies();
  const sessionCookie = cookieStore.get(SESSION_COOKIE_NAME); // Fixed: Await cookies() instead of .get()
...
export async function logoutUser() {
  ...
  const cookieStore = await cookies();
  await cookieStore.delete(SESSION_COOKIE_NAME, { path: '/' });
  redirect('/login');
}
