...
export async function loginUser(credentials) {
  ...
  try {
    ...
    // Surface more specific error info for debugging
    const isPrismaError = error.constructor?.name?.startsWith('Prisma') || error.code?.startsWith?.('P');
    if (isPrismaError) {
      logger.error('Prisma error code:', error.code);
      return { success: false, error: 'A database connection error occurred. Please try again later.' };
    }
