...
export async function calculateDistanceBetweenLocations(origin, destination) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[${timestamp}] [calculateDistanceBetweenLocations] HTTP error: ${response.status} ${response.statusText}`);
      return null;
    }
    ...
    console.error(`[${timestamp}] [calculateDistanceBetweenLocations] Error calling Google Maps API for origin "${origin}" to destination "${destination}":`, error.message);
    return null;
  }
}
